...
-- Adds Streak Tracking, Song Mastery, and Lifetime Milestones
-- ============================================================================
...
-- User streak data - stores per-user streak information
...
-- Daily practice log - aggregated practice per day
...
-- Streak freeze history - tracks when freezes are earned/used
CREATE TABLE IF NOT EXISTS streak_freeze_history (
  ...
  streak_day_count INTEGER, -- Day count when freeze was earned
  notes TEXT,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Song mastery progress - tracks skill tree progress per song
CREATE TABLE IF NOT EXISTS song_mastery_progress (
  ...
  -- Performance Path (5 nodes - future-ready placeholders)
  ...
  -- Computed star rating (0-3 based on completed paths)
...
-- Add song_structure column to songs table for theory tracking
ALTER TABLE songs ADD COLUMN IF NOT EXISTS song_structure JSONB DEFAULT NULL;
-- Example structure: {"intro": true, "verse": true, "chorus": true, "bridge": false, "outro": true}

-- Index for mastery table
...
-- Lifetime milestone definitions
CREATE TABLE IF NOT EXISTS lifetime_milestones (
  ...
  threshold_meta JSONB, -- Additional metadata (e.g., {"genre": "Rock"})
...
-- LIFETIME_MILESTONES policies (read-only for all authenticated users)
...
-- TIME MILESTONES (total practice seconds)
...
-- SONG MILESTONES (1 hour = mastered)
INSERT INTO lifetime_milestones (id, category, title, description, icon, threshold, tier, sort_order) VALUES
...
-- GENRE MILESTONES (5 mastered songs in genre)
...
BEGIN
  ...
  -- Update goal_met flag if needed
  IF v_goal_met AND NOT COALESCE(v_previous_goal_met, FALSE) THEN
    ...
    -- Calculate streak logic (only when first meeting goal today)
    ...
    -- Check if freeze is earned (every 7 days, max 3)
    ...
    RETURNING * INTO v_streak_data;
  ELSE
    -- Goal not met or was already met today - just return current state
    v_new_streak := v_streak_data.current_streak;
  END IF;
...
BEGIN
  ...
  WHERE s.user_id = p_user_id
    AND s.total_practice_seconds >= 3600  -- 1 hour = mastered
  GROUP BY g.genre;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- NOTES
-- ============================================================================
-- 1. user_streaks tracks daily practice goals and streak counts
-- 2. daily_practice_logs aggregates practice per day for calendar view
-- 3. streak_freeze_history tracks when freezes are earned/used/expired
-- 4. song_mastery_progress tracks skill tree nodes per song
-- 5. lifetime_milestones defines global achievement thresholds
-- 6. user_lifetime_milestones tracks which global milestones are unlocked
-- 7. update_user_streak RPC handles all streak logic atomically
-- 8. get_mastered_songs_by_genre helps check genre milestones
-- 9. All tables have RLS policies for security
-- 10. Note: songs.techniques is used as genre proxy until genre column is added
